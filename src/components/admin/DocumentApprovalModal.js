...
function DocumentApprovalForm() {
    ...
    useEffect(() => {
        const fetchRequestDetails = async () => {
            ...
            setLoading(true);
            try {
                const response = await fetch(`${API_BASE_URL}/requests/${requestId}`, { headers: { 'Authorization': `Bearer ${getToken()}` } });
    ...
    const handleFinalizeAndPrint = async () => {
        if (!request) return;
        try {
            await fetch(`${API_BASE_URL}/requests/${request.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${getToken()}` }, body: JSON.stringify({ status: 'ready for pick-up', notes: 'Document is ready.' }) });
        ...
        <div className="container-fluid my-4">
            <div className="card shadow-sm">
                ...
                <div className="card-footer text-end">
                    ...
                    {/* This button will now only show when isEditing is false */}
                    {!isEditing && (
